...
def getDown(x,y,matrix)
  out = 1
  for i in (0..3)
    if(y+i>matrix.column_size)
      return 0
    end
    out=out * matrix[x,y+i].to_i
  end
  return out

end

  def getRight(x,y,matrix)
    out = 1
    for i in (0..3)
      #puts matrix[x+i,y].to_i
      if(x+i>matrix.row_size)
        return 0
      end
      out=out * matrix[x+i,y].to_i
    end
    return out

  end

  def getDiagRight(x,y,matrix)
    out = 1
    for i in (0..3)
      if(x+i>matrix.row_size||y+i>matrix.column_size)
        return 0
      end
      #puts matrix[x+i,y+i].to_i
      out=out * matrix[x+i,y+i].to_i
    end
    return out
...
for i in 0..19
  for j in 0..19
    if getDown(i,j,grid) > max
      puts "new max D"
      max=getDown(i,j,grid)
    end
    if getRight(i,j,grid) > max
      puts "new max R"
      max=getDown(i,j,grid)
    end
    if getDiagRight(i,j,grid) > max
      puts "new max DR"
      max=getDiagRight(i,j,grid)
    end
    if getDiagLeft(i,j,grid) > max
      puts "new max DL"
      max=getDiagLeft(i,j,grid)
    end
